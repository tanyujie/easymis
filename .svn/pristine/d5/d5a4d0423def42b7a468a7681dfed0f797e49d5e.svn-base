...
public class StrUtils {
	private static String currentDay = new SimpleDateFormat("yyMMdd").format(new Date());
	private static BigDecimal  currentOrderId =new BigDecimal(new SimpleDateFormat("yyMMddhhmmss").format(new Date())+"0000");
	...
	*/}
	...
	public static String makeOrderId() {
		String orderId = "";
		if (currentDay.equals(new SimpleDateFormat("yyMMdd").format(new Date())))
			orderId = currentOrderId.add(new BigDecimal("1")).toString();
		else {
			orderId = new BigDecimal(new SimpleDateFormat("yyMMdd").format(new Date()) + "0001").toString();
		}
		currentOrderId = new BigDecimal(orderId);
		return orderId.trim();
	}

	public static String makeOrderId(String bizCode) {
		...
		currentOrderId = new BigDecimal(orderId);	
		return bizCode + orderId;
	}
	public static String makePk() {
		String pk = new BigDecimal(new SimpleDateFormat("yyyyMMddhhmmSSSS").format(new Date())).toString();
		return pk;
	}

	public static String makePayLogId() {
		String payLogId = new SimpleDateFormat("yyyyMMddhhmmSSS").format(new Date()).toString();

		return payLogId;
	}

	public static String makePayLogId(String bizCode) {
		String payLogId = new SimpleDateFormat("yyyyMMddhhmmSSS").format(new Date()).toString();
		return bizCode + payLogId;
	}
	public static void main(String args[]) {
		System.out.println(replaceKeyString("&nbsp;\r" + "</p>"));
		for (int i = 0; i < 1000; i++)
			System.out.println(makeOrderId());
