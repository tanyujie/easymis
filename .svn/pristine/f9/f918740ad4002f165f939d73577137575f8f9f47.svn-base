 ...
 * 
 */
package cn.easymis.commons.code.util.main;
...
import cn.easymis.commons.code.util.model.ModelMapperJava;
import cn.easymis.commons.code.util.model.ModelMapperXMLUtils;
	...
	public void  genFile(Class pojoclass,String filePath){
		...
		if(vo!=null){
			...
			if(keyValue!=null){
				...
				String intefaceJavaContent = msu.getModelServiceInterface(modelPackage,modelPackage, pojoClassName, modelName, idValues);
				...
				//service 实现类中的内容
				String implJavaContent = msu.getModelServiceImpl(modelPackage,modelPackage, pojoClassName, modelName, idValues);
...
/*				//生成页面
	...
	public void  genModelFolder(String filePath,String packageName,String modelName,String classType,String content){
		if(packageName!=null&&filePath!=null){
			...
			if(classType.trim().equals("action")){
				 ...
				 GenFile  genFile = new GenFile();
				 try {
					File file = genFile.createFile(packageNamePath+"/manager/impl", sb.toString()+"MngImpl.java");
				    genFile.writeFile(file, content); //向文件中写内容
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				 ...
				 GenFile  genFile = new GenFile();
				 try {
					File file = genFile.createFile(packageNamePath+"/manager", sb.toString()+"Mng.java");
				    genFile.writeFile(file, content); //向文件中写内容
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	public String getProjectRoot() {
		 ...
		 path = path.replace("com/ieker/code/util/main/", "");
	...
	public static void main(String[] arg) {
		...
		// 根据pojo对象生成代码
/*		Class pojoclass = FlowTaskStaff.class;
