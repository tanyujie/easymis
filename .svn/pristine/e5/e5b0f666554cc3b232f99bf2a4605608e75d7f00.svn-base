...
/*		if (auth && user != null && !user.isSuper() && mav != null
	...
	private CmsSite getByParams(HttpServletRequest request,
		...
		if (!StringUtils.isBlank(p)) {
			try {
				...
				CmsSite site = null;//cmsSiteMng.findById(siteId);
				if (site != null) {
		...
		if (cookie != null) {
			...
			if (!StringUtils.isBlank(v)) {
				try {
					...
					return null;//cmsSiteMng.findById(siteId);
				} catch (NumberFormatException e) {
	...
	private CmsSite getByDomain(HttpServletRequest request) {
		...
		if (!StringUtils.isBlank(domain)) {
			return null;//cmsSiteMng.findByDomain(domain);
		}
		return null;
	}

	private CmsSite getByDefault() {
		List<CmsSite> list = null;//cmsSiteMng.getListFromCache();
		if (list.size() > 0) {
			return list.get(0);
		} else {
			return null;
	...
	private static String getURI(HttpServletRequest request)
		...
		if (start <= 0) {
			throw new IllegalStateException(
					"admin access path not like '/iekeradmin/iekercms/...' pattern: "
							+ uri);
		}
	...
	private Set<String>getUserPermission(CmsSite site,CmsUser user){
		...
		Set<String> perms = null;//user.getPerms(site.getId(),viewPermissionSet);
