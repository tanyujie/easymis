package cn.easymis.finance.service;
...
import cn.easymis.entitys.mybatis.dto.FinanceCurrencyDto;
...
public class AppGenCodeMain {
	...
	public void  genFile(Class pojoclass,String filePath){
		...
		if(vo!=null){
			...
			String modelPackage = "cn.easymis.finance.service";
			...
			if(keyValue!=null){
				...
				String intefaceJavaContent = msu.getModelServiceInterface(filePath,modelPackage, pojoClassName, modelName, idValues);
				...
				//service 实现类中的内容
				String implJavaContent = msu.getModelServiceImpl(filePath,modelPackage, pojoClassName, modelName, idValues);
				...
				String actionJavaContent = mau.modelActionContent(modelPackage, pojoClassName, modelName, idValues);
	...
	public void  genModelFolder(String filePath,String packageName,String modelName,String classType,String content){
		if(packageName!=null&&filePath!=null){
			...
			if(classType.trim().equals("action")){
				 GenFile  genFile = new GenFile();
				 try {
					File file = genFile.createFile(packageNamePath+"/action/front", sb.toString()+"Act.java");
				    genFile.writeFile(file, content); //向文件中写内容
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				 ...
				 GenFile  genFile = new GenFile();
				 try {
					File file = genFile.createFile(packageNamePath+"/impl", sb.toString()+"MngImpl.java");
				    genFile.writeFile(file, content); //向文件中写内容
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				 ...
				 GenFile  genFile = new GenFile();
				 try {
					File file = genFile.createFile(packageNamePath+"/", sb.toString()+"Mng.java");
				    genFile.writeFile(file, content); //向文件中写内容
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				 ...
				 GenFile  genFile = new GenFile();
				 try {
					File file = genFile.createFile(packageNamePath+"/mapper", sb.toString()+"Mapper.xml");
				    genFile.writeFile(file, content); //向文件中写内容
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				 ...
				 GenFile  genFile = new GenFile();
				 try {
					File file = genFile.createFile(packageNamePath+"/mapper", sb.toString()+"Mapper.java");
				    genFile.writeFile(file, content); //向文件中写内容
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	public String getProjectRoot() {
		 URL url = AppGenCodeMain.class.getResource("");
		 String path = url.toString().replace("file:/", "");
		 path = path.replace("cn/easymis/finance/service/", "");
	...
	public static void main(String[] arg) {
		// 获取工程路径名称
		AppGenCodeMain gct = new AppGenCodeMain();
		String filePath = gct.getProjectRoot();
		// 根据pojo对象生成代码
		Class pojoclass = FinanceCurrencyDto.class;
		AppGenCodeMain gcm = new AppGenCodeMain();
		// 生成文件的路径名称
