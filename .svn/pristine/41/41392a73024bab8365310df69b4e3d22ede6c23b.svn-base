...
public class ModelPageUtils{
	...
	public String  getJspHeader(){
		...
		sb.append("<%@ taglib uri=\"http://www.tyeryj.com/limit\" prefix=\"limit\"%> \r\n");
	...
	public String getIndexJsp(List<FieldColumnPojo> list,String modelName){
	    ...
	    if(condition_list.size()>0){
	    	sb.append("<div style=\"float:left\"><form id=\"queryForm\" style=\"margin:10;\"><table width=\"100%\"><tr>");
	    }
		
	    //添加主页面中的条件
	    for(FieldColumnPojo obj : condition_list){
	    	  sb.append("<td  width=\"300px\">").append(obj.getLabelName()).append(":<input name=\""+obj.getFieldName()+"\" style=\"height: 20px\"></td>");
	    }
	    
	    if(condition_list.size()>0){
	    	sb.append("<td><input type=\"button\" name=\"search\" onclick=\"javascript:load_"+modelName+"grid(true);\" value='查询' />");
	    }
		...
		sb.append("<div style=\"float:right\"><div style=\"height:20px;line-height:20px;margin:10px;display:inline-block;\">");
		
		//添加权限标签
		sb.append("<limit:weight type=\"add\"><input type=\"button\" value=\"添加\" onclick=\"javascript:addupdate_"+modelName+"();\"/></limit:weight>");
	...
	public String indexScript(String modelName){
		...
		//加载分页的主方法
		sb.append("load_"+modelName+"grid = function(state){ \r\n if(state){ \r\n  beginRecord = 0; \r\n maxRecord = 20; \r\n }");
		
		//加载分页方法的ajax
        sb.append("$.ajax({type: \"POST\", \r\n url: '<%=path%>/"+modelName+"/grid.action?page='+currentPage+'&rows='+maxRecord, \r\n data:$(\"#queryForm\").serialize(), \r\n ");
		
        sb.append("success: function(response){ \r\n $(\"#"+modelName+"Grid\").html(response); \r\n  $.ligerDialog.closeWaitting(); \r\n },error:function(){ \r\n $.ligerDialog.error('服务器繁忙，请稍后再试');} \r\n }); \r\n };"+"\r\n");

        //分页的附加方法
        sb.append("pageselectCallback = function(page_id,perpage,jq){ \r\n beginRecord = page_id*perpage; \r\n  maxRecord   = perpage; \r\n currentPage = page_id; \r\n load_"+modelName+"grid(); \r\n };"+"\r\n");
        
        
        //更新或者添加方法
        sb.append("addupdate_"+modelName+" = function(id){ \r\n $.ligerDialog.open({ url: '<%=path%>/"+modelName+"/forwordEdit.action?id='+id, width:500,height:150, modal: true,title:'"+modelName+"'}); \r\n };"+"\r\n");
        
       
        //删除主方法
        sb.append("del_"+modelName+"=function(id){ \r\n $.ligerDialog.confirm({content:'确认删除这条数据吗?'}, function(r){ \r\n if(r==true){ \r\n");
        sb.append("$.ajax({type: \"POST\", \r\n url: '<%=path%>/"+modelName+"/delete.action', \r\n  data:'id='+id, \r\n  success: function(msg){  \r\n $.ligerDialog.success('删除成功'); \r\n");
        sb.append("load_"+modelName+"grid(false); \r\n },error:function(msg){  \r\n  $.ligerDialog.error('服务器繁忙，请稍后再试');} \r\n }); \r\n }}); \r\n };"+"\r\n");
	...
	public String  getGridContentJsp(List<FieldColumnPojo> list,String modelName){
		...
		sb.append("$(\"#pager\").pagination(\"${pageInfo.totalResult}\", {items_per_page : maxRecord ,current_page : currentPage,callback: pageselectCallback});");
    	...
    	sb.append("</tr></thead><tbody id=\""+modelName+"GridTbody\"><c:if test=\"${empty "+modelName+"_list }\"><tr>");
    	
    	sb.append("<td colspan=\"13\" align=\"center\"><spring:message code='grid.nodata'/></td></tr></c:if>");
    	
    	sb.append(" <c:forEach var=\""+modelName+"\" items=\"${"+modelName+"_list}\" varStatus=\"status\"><tr>");
    	...
    	for(FieldColumnPojo obj : list){
    		 if(!obj.getId()){
			    sb.append("  <td align=\"center\"><c:out value=\"${"+modelName+"."+obj.getFieldName()+"}\"/></td>");
    		 }else{ //如果有组件的话增加操作这列
    			 temp  = obj;
    		 }
		}
    	...
    	if(temp!=null){
	    	 sb.append("<td align=\"center\"><limit:weight type=\"update\"><a href=\"#\" onclick=\"javascript:addupdate_"+modelName+"('${"+modelName+"."+temp.getFieldName()+"}');return false;\">编辑</a></limit:weight>");
			
	    	 //删除信息
	    	 sb.append(" <limit:weight type=\"update\"><a href=\"#\" onclick=\"javascript:del_"+modelName+"('${"+modelName+"."+temp.getFieldName()+"}');return false;\">删除</a></limit:weight> ");
			 //查看信息
			 sb.append(" <a href=\"#\" onclick=\"javascript:view_"+modelName+"('${"+modelName+"."+temp.getFieldName()+"}');return false;\">查看</a></td>");
    	}
	...
	public String getEditJspContent(List<FieldColumnPojo> list,String modelName){
		...
		sb.append("<div align=\"center\" style=\"margin-top:10px;\"><form id=\"edit"+modelName+"Form\" method=\"post\"><table align=\"center\">");
		...
		for(FieldColumnPojo obj : list){
			...
			if(!obj.isPermitNull()){
				...
				sb.append("<td><input type=\"text\" id=\""+obj.getFieldName()+"\" name=\""+obj.getFieldName()+"\" value=\"${"+modelName+"_obj."+obj.getFieldName()+"}\"/></td></tr>");
			}else{
	    ...
	    sb.append(" <input type=\"button\" id=\"submitbutton\" name=\"submitbutton\" onclick=\"javascript:submit"+modelName+"Data();return false;\" value='提交'/>");
	    sb.append(" <input type=\"button\" name=\"reset\" value='取消' onclick=\"javascript:parent.$.ligerDialog.close();\"/>");
		sb.append("</div></td></tr>");
