	...
	public ModelAndView resolveException(HttpServletRequest request,
	   ...
	   //  e.printStackTrace();
	     ...
	     if(exceptionClass.equals(DataIntegrityViolationException.class)){
		     ...
		     String errorMsg=com.easymis.commons.web.WebErrors.create(request).getMessage(errorCode);
		     model.put("error",errorMsg); 
		     return new ModelAndView("/error/integrityViolation", model);
	     }else if(exceptionClass.equals(BadSqlGrammarException.class)){
		     String errorMsg=com.easymis.commons.web.WebErrors.create(request).getMessage("BadSqlGrammar");
		     model.put("error",errorMsg);
