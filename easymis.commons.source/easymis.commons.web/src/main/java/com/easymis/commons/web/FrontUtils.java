...
import static com.easymis.commons.web.Constants.MESSAGE;
import static com.easymis.commons.web.Constants.PROCESS_URL;
import static com.easymis.commons.web.Constants.RES_PATH;
import static com.easymis.commons.web.Constants.TPL_SUFFIX;
import static com.easymis.commons.web.ProcessTimeFilter.START_TIME;
...
import com.easymis.commons.web.springmvc.MessageResolver;
...
/*	*//**
	...
	*//**
	 ...
	 * @return
	 */
	public static String showRedirect(HttpServletRequest request,
			Map<String, Object> model, CmsSite site, String message,String url) {
		if (!StringUtils.isBlank(message)) {
			model.put(MESSAGE, message);
		}
		StringBuilder buff = new StringBuilder("redirect:");
	 ...
	 */
	public static String showRedirect(HttpServletRequest request,
			Map<String, Object> model, CmsSite site,String url) {
		return showRedirect(request, model, site, "true",url);
	}
	/**
	 * 为前台模板设置公用数据
	 * 
	...
	public static void frontData(HttpServletRequest request,
		...
		frontData(request,map, site, user, location, startTime);
/*		EcCart cart = (EcCart) request.getSession().getAttribute("cart");
		if(cart!=null){
			map.put("cartProdCount", cart.getRecords().size());
		}else{
			map.put("cartProdCount", "0");
		}*/
	}
	public static void frontData(HttpServletRequest request,
			Map<String, Object> map, CmsSite site,com.github.pagehelper.PageInfo p) {
		CmsUser user = CmsUtils.getUser(request);
		String location = RequestUtils.getLocation(request);
		Long startTime = (Long) request.getAttribute(START_TIME);
		...
		jsonObject.put("pageSize", p.getPageSize());
		jsonObject.put("pageNum", p.getPageNum());
		jsonObject.put("last_page", p.getLastPage());
		jsonObject.put("next_page_url", "");
		jsonObject.put("prev_page_url", "");
		jsonObject.put("startRow", p.getStartRow());
/*		EcCart cart = (EcCart) request.getSession().getAttribute("cart");
		if(cart!=null){
			map.put("cartProdCount", cart.getRecords().size());
		}else{
		...
		map.put("pageInfo", jsonObject.toString());
	}
	public static void frontData(Map<String, Object> map, CmsSite site,
		...
		map.put(SITE, site);
		String ctx = (site==null||site.getContextPath() == null) ? "" : site.getContextPath();
		//
		if(ctx==null||"".equals(ctx));
		ctx="http://127.0.0.1:50001/easymis/";
		map.put(BASE, ctx);
...
/*
	*//**
	...
	public static void frontPageData(HttpServletRequest request,
		...
		com.easymis.commons.web.URLHelper.PageInfo info = URLHelper.getPageInfo(request);
